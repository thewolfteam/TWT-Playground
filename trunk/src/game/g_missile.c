...
void G_RunMissile( gentity_t *ent ) {
	...
	// Perro: when mortar boxing is on, must be careful...
	// trap_Trace will detect a collision when a bolt with a z-axis box
    // re-enters the sky from above.  This will cause the mortar to explode in the sky.
	// To avoid this, remove the box only while the bolt is above the sky.  Replace it
	// once the bolt enters again.

	if ((ent->count2 == 1) && 
		(ent->s.weapon == WP_MORTAR_SET) && 
		(g_weapons.integer & WPF_MORTAR_BOXED))
	{
		VectorSet(ent->r.mins, 0, 0, 0);
		VectorSet(ent->r.maxs, 0, 0, 0);
	} else if ((ent->s.weapon == WP_MORTAR_SET) && 
		(g_weapons.integer & WPF_MORTAR_BOXED))
	{
		VectorSet(ent->r.mins, -20, 0, 0);
		VectorSet(ent->r.maxs, 20, 0, 40);
	}
...
void G_LandmineThink( gentity_t *self ) {
	...
	for( i = 0; i < cnt; i++) {
		...
		// perro - if option is set, don't blow up mines from my own team
		if ((g_friendlyFireOpts.integer & FFOPTS_NO_FRIENDLY_MINES) &&
			(G_LandmineTeam( self ) == ent->client->sess.sessionTeam )&& 
...
// jpw
    case WP_MORTAR_SET:
			...
			bolt->s.eFlags				= 0;
			if (g_weapons.integer & WPF_MORTAR_BOXED)
			{
			VectorSet(bolt->r.mins, -20, 0, 0);
			VectorCopy(bolt->r.mins, bolt->r.absmin);
			VectorSet(bolt->r.maxs, 20, 0, 40);
			VectorCopy(bolt->r.maxs, bolt->r.absmax);
			}
			break;
		case WP_LANDMINE:
...
//			bolt->r.ownerNum			= ENTITYNUM_WORLD;	// (SA) make the world the owner of the dynamite, so the player can shoot it without modifying the bullet code to ignore players id for hits
	...
	// perro - tunables: call G_GetWeaponDamageRadius to learn bolt->splashdamage
	bolt->splashRadius = G_GetWeaponDamageRadius(grenadeWPID);
...
gentity_t *fire_rocket (gentity_t *self, vec3_t start, vec3_t dir) {
	...
	int projectiles = 0;
	int i;
	int patternLen = strlen(g_partyPanzersPattern.string);

	VectorNormalize(dir);

	if (g_partyPanzersEnabled.integer) {
		// Traverse pattern to count lines and projectiles
		for (i = 0; i < patternLen; i++) {
			if (g_partyPanzersPattern.string[i] == '1') projectiles++;
			if (g_partyPanzersPattern.string[i] == ';') lines++;
			if (g_partyPanzersPattern.string[i] == ',') lines++;
		}
	}

	// Only do party panzers if enabled and we have to fire at least
	// one projectile, otherwise fire a normal panzer
	if (projectiles >= 1) {
		int projDamage = 0;
		int ii;
		char currChar;
		vec_t currHorDelta = 0;
		vec_t currVertDelta;
		int newLineFlag = 1;
		int slotCount;
		vec3_t angles, forward, right, up, newForward;

		// tjw: do not use up all game entities with rockets, it will 
		//      cause a server stopping error
		if(projectiles > 
			(G_GentitiesAvailable() - MIN_SPARE_GENTITIES)) {
			
			CPx(self-g_entities,
				"print \"Sorry, there are not enough free "
				"game entites to fire the Party Panzer."
				"Try again later.\n\"");
			return NULL;
		}

		// Calculate damage per projectile
		switch(g_partyPanzersDamageMode.integer) {
			case PP_DAMAGE_FULL:
				projDamage = G_GetWeaponDamage(WP_PANZERFAUST);
				break;
			case PP_DAMAGE_PROPORTIONAL:
				projDamage = G_GetWeaponDamage(WP_PANZERFAUST) / projectiles;
				break;
		}

		// Calculate offset of the first line
		currVertDelta = -PP_SPACING * (lines / 2);

		// We will add this back in the first iteration of the loop
		currVertDelta -= PP_SPACING;

		vectoangles(dir, angles);
		AngleVectors(angles, forward, right, up);

		// Traverse pattern again and spawn projectiles
		for (i = 0; i <= patternLen; i++) {
			currChar = g_partyPanzersPattern.string[i];

			if (newLineFlag) {
				currVertDelta += PP_SPACING;

				// Count the number of "slots" in the current line's pattern
				// and calculate the X coordinate of the first projectile
				slotCount = 0;
				for(ii = i;
					(g_partyPanzersPattern.string[ii] != ';') &&
					(g_partyPanzersPattern.string[ii] != ',') &&
					(ii < patternLen); ii++) {

					if ((g_partyPanzersPattern.string[ii] == '1') ||
						(g_partyPanzersPattern.string[ii] == '0')) slotCount++;
				}
				currHorDelta = -PP_SPACING * (slotCount / 2);
			...
			switch (currChar) {
				case '1':
					...
					if (g_panzersVulnerable.integer) {
						...
						VectorSet(bolt->r.mins, -3, -3, 0);
						VectorCopy(bolt->r.mins, bolt->r.absmin);
						VectorSet(bolt->r.maxs, 3, 3, 6);
						VectorCopy(bolt->r.maxs, bolt->r.absmax);
					}
					...
					bolt->r.ownerNum = self->s.number;
					bolt->parent = self;
					bolt->damage = projDamage;
					bolt->splashDamage = projDamage;
					//perro: tunable (default= previously hardcoded value)
					//bolt->splashRadius = 300; //G_GetWeaponDamage(WP_PANZERFAUST);	// Arnout : hardcoded bleh hack
					bolt->splashRadius = G_GetWeaponDamageRadius(WP_PANZERFAUST);
					...
					// Update the direction of this panzer shot
					VectorMA(forward, currHorDelta, right, newForward);
					VectorMA(newForward, currVertDelta, up, newForward);

					// Set velocity
					VectorNormalize(newForward);
					VectorScale(newForward, g_panzersSpeed.integer, bolt->s.pos.trDelta);
					...
					// No break; here, we're supposed to fall through
				case '0':
					currHorDelta += PP_SPACING;
					break;
				case ',':
				case ';':
					newLineFlag = 1;
