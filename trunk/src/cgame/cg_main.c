...
vmCvar_t	cg_ignore;
vmCvar_t	cg_fov;
//vmCvar_t	cg_zoomFov;				// UNUSED
//vmCvar_t	cg_zoomStepBinoc;		// UNUSED
vmCvar_t	cg_zoomStepSniper;
//vmCvar_t	cg_zoomStepSnooper;		// UNUSED
//vmCvar_t	cg_zoomStepFG;		//----(SA)	added		// UNUSED
vmCvar_t	cg_zoomDefaultBinoc;
vmCvar_t	cg_zoomDefaultSniper;
//vmCvar_t	cg_zoomDefaultSnooper;	// UNUSED
//vmCvar_t	cg_zoomDefaultFG;	//----(SA)	added		// UNUSED
...
//vmCvar_t	cg_deferPlayers;		// UNUSED
//vmCvar_t	cg_drawTeamOverlay;		// UNUSED
vmCvar_t	cg_enableBreath;
vmCvar_t	cg_autoactivate;
//vmCvar_t	cg_blinktime;	//----(SA)	added	// UNUSED

//vmCvar_t 	cg_smoothClients;		// UNUSED
...
//vmCvar_t	cg_messagePlayer;		// UNUSED
//vmCvar_t	cg_messagePlayerName;	// UNUSED
vmCvar_t	cg_movespeed;
vmCvar_t	cg_cameraMode;
//vmCvar_t	cg_cameraOrbit;		// UNUSED
//vmCvar_t	cg_cameraOrbitDelay;	// UNUSED
//vmCvar_t	cg_timescaleFadeEnd;	// UNUSED
//vmCvar_t	cg_timescaleFadeSpeed;	// UNUSED
...
//vmCvar_t	cg_animState;			// UNUSED

vmCvar_t	cg_drawCompass;
//vmCvar_t	cg_drawNotifyText;		// UNUSED
vmCvar_t	cg_quickMessageAlt;
//vmCvar_t	cg_popupLimboMenu;		// UNUSED
...
vmCvar_t cg_font3;
vmCvar_t cg_font4;
vmCvar_t cg_font1size;
vmCvar_t cg_font2size;
vmCvar_t cg_font3size;
vmCvar_t cg_font4size;
vmCvar_t cg_fontLimboScale;
vmCvar_t cg_fontCPScale;
vmCvar_t cg_fontOIDScale;
vmCvar_t cg_fontQAScale;
vmCvar_t cg_fontBPScale;

// forty - unlagged optimized prediction
vmCvar_t cg_optimizePrediction;


// tjw: ratings scoreboard
vmCvar_t cg_scoreboard;
vmCvar_t cg_scoresDoubleTap;
...
// HOLLEMOD - END
vmCvar_t	cg_watermarkAlpha;

// Dens: banner/centerprint logging
vmCvar_t	cg_logConsole;

vmCvar_t	cg_weapaltReloads;

// Dens: 0 = No, 1 = Full (default), 2 = Short
vmCvar_t	cg_drawRanks;
vmCvar_t	cg_drawClassIcons;
vmCvar_t	cg_countryFlags; //mcwf GeoIP
vmCvar_t	cg_drawCrosshairHP;

// Dens: Dyno counter
vmCvar_t	cg_dynoCounter;
...
//	{ &cg_zoomFov, "cg_zoomfov", "22.5", CVAR_ARCHIVE },					// UNUSED
//	{ &cg_zoomDefaultBinoc, "cg_zoomDefaultBinoc", "22.5", CVAR_ARCHIVE },	// UNUSED
	{ &cg_zoomDefaultSniper, "cg_zoomDefaultSniper", "20", CVAR_ARCHIVE }, // JPW NERVE changed per atvi req
//	{ &cg_zoomDefaultSnooper, "cg_zoomDefaultSnooper", "40", CVAR_ARCHIVE }, 	// UNUSED
//	{ &cg_zoomDefaultFG, "cg_zoomDefaultFG", "55", CVAR_ARCHIVE },			// UNUSED
//	{ &cg_zoomStepBinoc, "cg_zoomStepBinoc", "3", CVAR_ARCHIVE },			// UNUSED
	{ &cg_zoomStepSniper, "cg_zoomStepSniper", "2", CVAR_ARCHIVE },
//	{ &cg_zoomStepSnooper, "cg_zoomStepSnooper", "5", CVAR_ARCHIVE },		// UNUSED
	//	{ &cg_zoomStepFG, "cg_zoomStepFG", "10", CVAR_ARCHIVE },			// UNUSED
...
//	{ &cg_drawStatus, "cg_drawStatus", "1", CVAR_ARCHIVE  },		// UNUSED
...
//	{ &cg_messagePlayer, "cg_messagePlayer", "", CVAR_TEMP },			// UNUSED
//	{ &cg_messagePlayerName, "cg_messagePlayerName", "", CVAR_TEMP },	// UNUSED
...
//	{ &cg_deferPlayers, "cg_deferPlayers", "1", CVAR_ARCHIVE },			// UNUSED
//	{ &cg_drawTeamOverlay, "cg_drawTeamOverlay", "2", CVAR_ARCHIVE },	// UNUSED
	{ &cg_stats, "cg_stats", "0", 0 },
//	{ &cg_blinktime, "cg_blinktime", "100", CVAR_ARCHIVE},				// UNUSED

	// Dens: why would we want to have a client cvar in serverinfo?
	{ &cg_enableBreath, "cg_enableBreath", "1", /*CVAR_SERVERINFO*/ 0},
//	{ &cg_cameraOrbit, "cg_cameraOrbit", "0", CVAR_CHEAT},				// UNUSED
//	{ &cg_cameraOrbitDelay, "cg_cameraOrbitDelay", "50", CVAR_ARCHIVE},	// UNUSED
//	{ &cg_timescaleFadeEnd, "cg_timescaleFadeEnd", "1", 0},				// UNUSED
//	{ &cg_timescaleFadeSpeed, "cg_timescaleFadeSpeed", "0", 0},			// UNUSED
	{ &cg_timescale, "timescale", "1", 0},
//	{ &cg_smoothClients, "cg_smoothClients", "0", CVAR_USERINFO | CVAR_ARCHIVE},		// UNUSED
...
//	{ &cg_smallFont, "ui_smallFont", "0.25", CVAR_ARCHIVE},					// UNUSED
//	{ &cg_bigFont, "ui_bigFont", "0.4", CVAR_ARCHIVE},						// UNUSED
...
//	{ &cg_drawNotifyText, "cg_drawNotifyText", "1", CVAR_ARCHIVE },		// DOES NOTHING
	{ &cg_quickMessageAlt, "cg_quickMessageAlt", "0", CVAR_ARCHIVE },
//	{ &cg_popupLimboMenu, "cg_popupLimboMenu", "1", CVAR_ARCHIVE },		// UNUSED
...
//	{ &cg_fastSolids,		"cg_fastSolids",	"1",	CVAR_ARCHIVE },			// UNUSED
	...
	{ &cg_panzerhack, "cg_panzerhack", "1", CVAR_ARCHIVE },
	...
	{ &cg_font4, "cg_font4", "courbd", CVAR_ARCHIVE },
	{ &cg_font1size, "cg_font1size", "27", CVAR_ARCHIVE },
	{ &cg_font2size, "cg_font2size", "16", CVAR_ARCHIVE },
	{ &cg_font3size, "cg_font3size", "30", CVAR_ARCHIVE },
	{ &cg_font4size, "cg_font4size", "21", CVAR_ARCHIVE },
	...
	{ &cg_scoresDoubleTap, "cg_scoresDoubleTap", "1", CVAR_ARCHIVE },
	...
	// forty - #297 - watermark
	// HOLLEMOD - START
	{ &g_watermark, "g_watermark", "", 0 },
	{ &g_watermarkFadeAfter, "g_watermarkFadeAfter", "0", 0 },
	{ &g_watermarkFadeTime, "g_watermarkFadeTime", "0", 0 },
	// HOLLEMOD - END
	{ &cg_watermarkAlpha, "cg_watermarkAlpha", "1.0", CVAR_ARCHIVE},

	// forty - Phoenix - scale weapon for custom FOV settings
	{ &cg_gun_fovscale, "cg_gun_fovscale", "0", CVAR_ARCHIVE	},
	...
	// Hardware address
	{ NULL , "mac", "", CVAR_USERINFO | CVAR_ROM }, //harald
};
		...
		if(cv->vmCvar) {
			...
			if(cv->modificationCount != cv->vmCvar->modificationCount) {
				...
				// HOLLEMOD - START
					...
					cv->vmCvar == &g_watermark ||
					cv->vmCvar == &g_watermarkFadeAfter ||
					cv->vmCvar == &g_watermarkFadeTime
				) {
					cgs.media.watermark = trap_R_RegisterShaderNoMip( va( "watermark/%s", g_watermark.string ) );
					// Reset connection time of watermark
...
qboolean CG_BackupProfile(void)
{
	char profile[MAX_CVAR_VALUE_STRING];
	fileHandle_t cfg, bak;
	char cfgPath[256], bakPath[256];
	int len = 0, i = 0;

	// tjw: back up etconfig.cfg so that forcecvar changes are never
	//      archived despite CVAR_ARCHIVE
	...
	// tjw: only create a new backup if one does not exist already
	if(len == 0) {
		len = trap_FS_FOpenFile(cfgPath, &cfg, FS_READ);
		if(len <= 0) {
			CG_Printf("forcecvar: could not read "
				"\"%s\" for backup\n", cfgPath);
			return qfalse;
		}
		if(trap_FS_FOpenFile(bakPath, &bak, FS_WRITE) < 0) {
			CG_Printf("forcecvar: could not write "
				"\"%s\" for backup\n", bakPath);
			trap_FS_FCloseFile(cfg);
			return qfalse;
		}
		for(i = 0; i < len; i++) {
			byte b;
			trap_FS_Read(&b, 1, cfg);
			trap_FS_Write(&b, 1, bak);
		}
		trap_FS_FCloseFile(cfg);
		trap_FS_FCloseFile(bak);
	}
...
void CG_RestoreProfile(void)
{
	...
	if(len > 0) {
		...
		for(i = 0; i < len; i++) {
			byte b;
			trap_FS_Read(&b, 1, bak);
			trap_FS_Write(&b, 1, cfg);
		}
		trap_FS_FCloseFile(cfg);
		trap_FS_FCloseFile(bak);
...
void QDECL CG_Printf( const char *msg, ... ) {
	...
	va_end (argptr);
	/*	// kw: Notify text drawing doesn't work so I disabled this.
	...
	// kw: eat [cgnotify] off the string
	if ( !Q_strncmp( text, "[cgnotify]", 10 ) ) {
		trap_Print( &text[10] );
		return;
	}
...
int CG_findClientNum(char *s)
{
	...
	// CHRUKER: b075 - Added the (int) type casting
...
//	cgs.media.cursor = trap_R_RegisterShaderNoMip( "ui/assets/mvcursor.tga" );
	...
	// forty - shader added, icon no longer shows through walls.
...
//	cgs.media.hflakWeaponSnd =		trap_S_RegisterSound( "sound/vehicles/misc/20mm_fire.wav", qfalse );
	...
	// forty - shader added this no longer shows through walls.
	cgs.media.disconnectIcon =					trap_R_RegisterShaderNoMip( "gfx/2d/net" );

	// forty - transparent gun models
	cgs.media.transGunWhite =					trap_R_RegisterShader( "textures/sfx/transgunWhite" );
	cgs.media.transGunRed =						trap_R_RegisterShader( "textures/sfx/transgunRed" );
	cgs.media.transGunGreen =					trap_R_RegisterShader( "textures/sfx/transgunGreen" );
	cgs.media.transGunBlue =					trap_R_RegisterShader( "textures/sfx/transgunBlue" );
...
//	int startat = trap_Milliseconds();
	...
	//      loading screen
	if(!CG_LoadFont(1, cg_font1.string, cg_font1size.integer))
		CG_LoadFont(1, "ariblk", 27);
	if(!CG_LoadFont(2, cg_font2.string, cg_font2size.integer))
		CG_LoadFont(2, "ariblk", 16);
	if(!CG_LoadFont(3, cg_font3.string, cg_font3size.integer))
		CG_LoadFont(3, "courbd", 30);
	if(!CG_LoadFont(4, cg_font4.string, cg_font4size.integer))
	...
	// forty - #297 - try to load the watermark initially
	cgs.media.watermark = trap_R_RegisterShaderNoMip( va( "watermark/%s", g_watermark.string ) );
	...
	if(CG_LoadHud(cg_hud.string)) {
		...
		CG_Printf("^2No HUD specified, default HUD loaded\n");
	} else {
