...
//	qboolean	playerScaled;
	...
	if( cent->currentState.eFlags & EF_MOUNTEDTANK ) {	
		...
		// forty - muzzleflash
		if( cg.time - cent->muzzleFlashTime < MUZZLE_FLASH_TIME && cg_muzzleFlash.integer) {
...
/*	playerScaled = (qboolean)(cgs.clientinfo[ cent->currentState.clientNum ].playermodelScale[0] != 0);
	...
	if( drawpart ) {
		...
		if( isPlayer && !cg.renderingThirdPerson && cg_drawGun.integer >= 2) {
			switch(cg_drawGun.integer) {

				case 3:
					gun.customShader = cgs.media.transGunRed;
					break;

				case 4:
					gun.customShader = cgs.media.transGunGreen;
					break;

				case 5:
					gun.customShader = cgs.media.transGunBlue;
					break;

				case 6:
					gun.customShader = cgs.media.genericConstructionShader;
					break;

				default:
					gun.customShader = cgs.media.transGunWhite;
					break;
				...
				if( drawpart ) {
					...
					if( isPlayer && !cg.renderingThirdPerson && cg_drawGun.integer >= 2) {
						switch(cg_drawGun.integer) {
							...
							case 3:
								barrel.customShader = cgs.media.transGunRed;
								break;

							case 4:
								barrel.customShader = cgs.media.transGunGreen;
								break;

							case 5:
								barrel.customShader = cgs.media.transGunBlue;
								break;

							case 6:
								barrel.customShader = cgs.media.genericConstructionShader;
								break;

							default:
								barrel.customShader = cgs.media.transGunWhite;
								break;
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	if ( ps->eFlags & EF_MG42_ACTIVE || ps->eFlags & EF_AAGUN_ACTIVE ) {
		CG_Printf("Mounted MG42\n");
		return;
	}

	// drop gun lower at higher fov
	// Phoenix - we don't want to drop the gun if we're scaling the axis 
	...
	if( ps->eFlags & EF_MOUNTEDTANK ) {
		...
		if( cg_drawGun.integer >= 2) {
			switch(cg_drawGun.integer) {
				...
				case 3:
					hand.customShader = cgs.media.transGunRed;
					break;

				case 4:
					hand.customShader = cgs.media.transGunGreen;
					break;

				case 5:
					hand.customShader = cgs.media.transGunBlue;
					break;

				case 6:
					hand.customShader = cgs.media.genericConstructionShader;
					break;

				default:
					hand.customShader = cgs.media.transGunWhite;
					break;
...
//----(SA)	removed
		...
		// Phoenix - scale weapon for custom FOV settings
...
int CG_WeaponIndex( int weapnum, int *bank, int *cycle) {
	...
	if( cg_panzerhack.integer && (
		...
		weapBanksMultiPlayer[3][10] = WP_MP40;
		weapBanksMultiPlayer[3][11] = WP_THOMPSON;
	}
...
void CG_AltWeapon_f(void)
{
	...
	// kw: follow the previous client
	if(cg.snap->ps.pm_flags & PMF_FOLLOW) {
		trap_SendConsoleCommand( "followprev" );
		return;
	}

	// forty - don't allow altfire while the M7/GPG40 are firing.
	//		   otherwise we end up with a jammed up rifle grenade in the heat of battle.
	if( 
		(cg.snap->ps.weapon == WP_M7 || cg.snap->ps.weapon == WP_GPG40) &&
	...
	else if (cg.weaponSelect == WP_MEDIC_SYRINGE) {
		// josh: adren other if supported
		if (cgs.adrenother) 
			trap_SendConsoleCommand("adrenother\n");
	}else if(cg.weaponSelect == WP_GRENADE_LAUNCHER ||
		cg.weaponSelect == WP_GRENADE_PINEAPPLE ||
		cg.weaponSelect == WP_SMOKE_MARKER){
		// Dens: inform the team of a nearby explosion
		trap_SendConsoleCommand("cmd vsay_team FireInTheHole\n");
	}
	...
	if((reload == qtrue) && cg_weapaltReloads.integer){
		// Dens: I hate it this way, but it seems to work. 
...
void CG_NextWeap(qboolean switchBanks) {
	...
	// forty - fix cycle weapon while mounted crash
	if( cg.snap->ps.persistant[PERS_HWEAPON_USE] || cg.snap->ps.eFlags & EF_MOUNTEDTANK ) {
		return;
	}
...
Returns the best weapon currently available,
can optionally ignore a specified weapon. 
===================
*/
int CG_GetBestWeapon( int ignoreWeap ) {
	int i;

	//look inside weaponbank 3 first
	for( i = 0; i < MAX_WEAPS_IN_BANK_MP; i++ ) {
		if( weapBanksMultiPlayer[3][i] != ignoreWeap
				&& CG_WeaponSelectable(weapBanksMultiPlayer[3][i])) {
			return weapBanksMultiPlayer[3][i];
		}
	}
	//look inside weaponbank 2
	for( i = 0; i < MAX_WEAPS_IN_BANK_MP; i++ ) {
		if( weapBanksMultiPlayer[2][i] != ignoreWeap
				&& CG_WeaponSelectable(weapBanksMultiPlayer[2][i])) {
			return weapBanksMultiPlayer[2][i];
		}
	}
	//look for a grenade (weaponbank 4)
	for( i = 0; i < MAX_WEAPS_IN_BANK_MP; i++ ) {
		if( weapBanksMultiPlayer[4][i] != ignoreWeap
				&& CG_WeaponSelectable(weapBanksMultiPlayer[4][i])) {
			return weapBanksMultiPlayer[4][i];
		}
	}
	
	//use the knife if available
	if(CG_WeaponSelectable(WP_KNIFE)) {
		return WP_KNIFE;
	} else {
		// kw: CG_FinishWeaponChange handles this correctly 
		//     by selecting the nextweapon.
		return WP_NONE;
	}
...
void CG_OutOfAmmoChange( qboolean allowforceswitch ) {
	...
	// trivial switching
	//
	if( cg.weaponSelect == WP_PLIERS || cg.weaponSelect == WP_KNIFE) {
		return;
	}

	if( allowforceswitch ) {

		// kw: code cleaned up into a switch statement
		switch(cg.weaponSelect) {
		case WP_SMOKE_BOMB:
			// kw: perhaps also select bestweapon here instead of always 
			//     the pistol?
		...
		case WP_SATCHEL:
			...
			break;
		case WP_MORTAR_SET:
			CG_FinishWeaponChange(cg.predictedPlayerState.weapon, WP_MORTAR);
			return;
		case WP_MOBILE_MG42_SET:
			CG_FinishWeaponChange(cg.predictedPlayerState.weapon, 
										WP_MOBILE_MG42);
		...
		case WP_PANZERFAUST:
			CG_FinishWeaponChange(cg.predictedPlayerState.weapon, 
										CG_GetBestWeapon( WP_PANZERFAUST ) );
			return;
		default:
			break;
		}
	...
	// kw: just select the best weapon available
	CG_FinishWeaponChange(cg.predictedPlayerState.weapon, 
							CG_GetBestWeapon( cg.weaponSelect ) );
}
