...
//Centerprint default height
#define CP_DEFAULTHEIGHT	395
#define CP_HIGHERHEIGHT		360		// little higher for more important messages
...
#define AA_DEMOSTOPWATCH	0x08	//KW 28-09-2005: only record stopwatch matches
...
// Dens: user can use cg_max(Team/Enemy)Dynas. This is the maximumof those CVARS
#define MAXDYNAMITE		8

typedef struct dynamite_s {
	int			entityNum;
	int			planted;		// the time it was planted
	//vec3_t		origin;			// the location
	// Dens: we don't need the team anymore
	//int			team;			// who does this belong to?
	char	pos[6];
} dynamite_t;
...
typedef struct hud_s {
	int healthbar[3];		// x, y, width
	int staminabar[3];		// x, y, width
	int chargebar[3];		// x, y, width
	int compass[3];			// x, y, size
	int hp[3];			// x, y, scale
	int xp[3];			// x, y, scale
	int draws[1];			// y
	int skillpics[3][3];		// x, y, size {3}
	int skilltexts[3][3];		// x, y, size {3}
	int skillboxes[3][3];		// x, y, size {3}
	int overheat[4];		// x, y, width, height
	int weaponcard[3];		// x, y, width
	int fireteam[3];		// x, y, width
	int ammo[3];			// x, y, scale
	int head[4];			// x, y, width, height
	int lagometer[2];		// x, y
	int flagcov[2];			// x, y
	int cpmtext[3];			// x, y, scale
	int chattext[3];		// x, y, scale
	int votefttext[2];		// x, y
	int livesleft[3];		// x, y, scale
} hud_t;

typedef struct hudEditor_s {
	qboolean	showHudEditor;
	qboolean	showCoor;
	qboolean	grouping;
	qboolean	saving;
	qboolean	loading;
	int		showHudEditorHelp;
	int		hudScaleMode;
	int		fadeTime;
	int		chatTime;
	int		popupTime;
...
typedef struct HudrectDef_s {
	float x;	// horiz position
	float y;	// vert position
	float w;	// width
	float h;	// height;
	qboolean initiazlied;
	int scaled;
	qboolean dragged;
	qboolean hover;
	qboolean grouped;
...
// forty - visual hud editor
typedef enum hudScaleModes_e {
	HUD_SCALE_NONE,
	HUD_SCALE_XY,
	HUD_SCALE_X,
	HUD_SCALE_Y,
	HUD_SCALE_UNIFORM_UP,
	HUD_SCALE_UNIFORM_DOWN,
} hudScaleModes_t;

// forty - visual hud editor
typedef struct {
	HudrectDef_t healthbar;
	HudrectDef_t staminabar;
	HudrectDef_t chargebar;
	HudrectDef_t compass;
	HudrectDef_t hp;
	HudrectDef_t xp;
	HudrectDef_t draws;
	HudrectDef_t skillpics[3];
	HudrectDef_t skilltexts[3];
	HudrectDef_t skillboxes[3];
	HudrectDef_t overheat;
	HudrectDef_t weaponcard;
	HudrectDef_t fireteam;
	HudrectDef_t ammo;
	HudrectDef_t head;
	HudrectDef_t lagometer;
	HudrectDef_t flagcov;
	HudrectDef_t cpmtext;
	HudrectDef_t chattext;
...
typedef struct {
	...
	qboolean	demoRecording;		// recording a demo
	...
	char		centerPrintLast[1024];		// Dens: need for centerprint logging
	int			centerPrintLastTime;
	...
	float		crosshairClientScanDistance;
...
// JPW FIXME NOTE: max_weap_banks > max_weap_banks_mp so this should be OK, but if that changes, change this too
	...
	float axisProb;
	float alliesProb;
	float playerRating[MAX_CLIENTS];
	float killRating[MAX_CLIENTS];

	// forty - bacon's dyno counter
	// Dens: changed into allied and axis to better fit teamchanges
	// Dens: 0 = axis, 1 = Allies
	...
	// HOLLEMOD - START
	int				watermarkConnectTime;
	// HOLLEMOD - END

	// forty - #601 - Merge Jaquboss' client-side hitbox prediction code
	// jaquboss - tracing bullet, predict hitboxes used on server
...
//	skill_testicon
	...
	qhandle_t		transGunWhite;
	qhandle_t		transGunRed;
	qhandle_t		transGunGreen;
	qhandle_t		transGunBlue;
...
typedef struct {
	...
	char			team_maxPanzers[10];
	char			team_maxMortars[10];
	char			team_maxFlamers[10];
	...
	char			skill_fieldops[MAX_CVAR_VALUE_STRING];
	char			skill_covertops[MAX_CVAR_VALUE_STRING];
	char			skill_battlesense[MAX_CVAR_VALUE_STRING];
	char			skill_lightweapons[MAX_CVAR_VALUE_STRING];
...
//	qboolean			playedLimboMusic;
	...
	int coverts; // see COVERTF_ defines
	int medics;  // see MEDIC_ defines
	int throwableKnives;
	qboolean adrenother;
	int playerRating;
	int killRating;
...
//extern	vmCvar_t		cg_drawTeamOverlay;		// UNUSED
extern	vmCvar_t		cg_crosshairX;
extern	vmCvar_t		cg_crosshairY;
extern	vmCvar_t		cg_crosshairSize;
extern	vmCvar_t		cg_crosshairHealth;
//extern	vmCvar_t		cg_drawStatus;			// UNUSED
...
//extern	vmCvar_t		cg_zoomFov;			// UNUSED
//extern	vmCvar_t		cg_zoomDefaultBinoc;	// UNUSED
extern	vmCvar_t		cg_zoomDefaultSniper;
//extern	vmCvar_t		cg_zoomDefaultFG;		// UNUSED
//extern	vmCvar_t		cg_zoomDefaultSnooper;	// UNUSED
//extern	vmCvar_t		cg_zoomStepBinoc;		// UNUSED
extern	vmCvar_t		cg_zoomStepSniper;
//extern	vmCvar_t		cg_zoomStepSnooper;		// UNUSED
//extern	vmCvar_t		cg_zoomStepFG;			// UNUSED
...
//extern	vmCvar_t 		cg_forceModel;			// UNUSED
...
//extern	vmCvar_t		cg_deferPlayers;		// UNUSED
...
//extern	vmCvar_t		cg_cameraOrbit;		// UNUSED
//extern	vmCvar_t		cg_cameraOrbitDelay;	// UNUSED
//extern	vmCvar_t		cg_timescaleFadeEnd;	// UNUSED
//extern	vmCvar_t		cg_timescaleFadeSpeed;	// UNUSED
extern	vmCvar_t		cg_timescale;
extern	vmCvar_t		cg_cameraMode;
//extern  vmCvar_t		cg_smallFont;			// UNUSED
//extern  vmCvar_t		cg_bigFont;			// UNUSED
extern	vmCvar_t		cg_noTaunt;		// NERVE - SMF
extern	vmCvar_t		cg_voiceSpriteTime;		// DHM - Nerve

//extern	vmCvar_t		cg_blinktime;	//----(SA)	added	// UNUSED
...
//extern vmCvar_t			cg_animState;		// UNUSED

extern vmCvar_t			cg_drawCompass;
//extern vmCvar_t			cg_drawNotifyText;	// UNUSED
extern vmCvar_t			cg_quickMessageAlt;
//extern vmCvar_t			cg_popupLimboMenu;	// UNUSED
...
//extern vmCvar_t			cg_announcer;		// UNUSED
...
//extern vmCvar_t			cg_specSwing;		// UNUSED
extern vmCvar_t			cg_uinfo;
extern vmCvar_t			cg_useScreenshotJPEG;
//extern vmCvar_t			ch_font;			// UNUSED
...
extern vmCvar_t	cg_specAlpha;

// tjw
extern vmCvar_t cg_font1;
extern vmCvar_t cg_font2;
extern vmCvar_t cg_font3;
extern vmCvar_t cg_font4;
extern vmCvar_t cg_font1size;
extern vmCvar_t cg_font2size;
extern vmCvar_t cg_font3size;
extern vmCvar_t cg_font4size;

extern vmCvar_t cg_fontLimboScale;
extern vmCvar_t cg_fontCPScale;
extern vmCvar_t cg_fontOIDScale;
extern vmCvar_t cg_fontQAScale;
extern vmCvar_t cg_fontBPScale;
...
extern vmCvar_t cg_scoresDoubleTap;
...
// HOLLEMOD - END
extern	vmCvar_t		cg_watermarkAlpha;
...
// Dens: dyno counter
extern	vmCvar_t		cg_dynoCounter;
extern	vmCvar_t		cg_maxTeamDynas;
extern	vmCvar_t		cg_maxEnemyDynas;
...
#define SCOREBOARD_PR 1
#define SCOREBOARD_KR 2

// Dens: cg_drawClassIcons
#define CLASSICON_CROSSHAIR 1
...
void CG_BannerPrint( const char *banner );
...
// OSP
#define Pri(x) CG_Printf("%s", CG_LocalizeServerCommand(x))
#define CPri(x) CG_PriorityCenterPrint(CG_LocalizeServerCommand(x), CP_HIGHERHEIGHT, SMALLCHAR_WIDTH, 99);
...
void CG_MapVote_SetSelectedMap( int mapNum );
...
void CG_Debriefing2_Title_Draw( panel_button_t* button );
void CG_Debriefing2_MissionTitle_Draw( panel_button_t* button );
void CG_Debriefing2_Mission_Draw( panel_button_t* button );
void CG_Debriefing2_Maps_Draw( panel_button_t* button );
void CG_Debriefing2_Awards_Draw( panel_button_t* button );
